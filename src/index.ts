 ...
 * - list_versions: version history with publish dates, dist-tags, deprecations
 * - get_version_info: metadata for a specific version / dist-tag
 */
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'npm');
}
...
const tools: McpToolExport['tools'] = [
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        name: { type: 'string', description: 'Exact package name (e.g. "express", "@types/node").' },
        limit: { type: 'number', description: 'Number of recent versions to return (default 20, max 100).' },
      },
      required: ['name'],
    },
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        name: { type: 'string', description: 'Exact package name (e.g. "express").' },
        version: { type: 'string', description: 'Exact version (e.g. "4.18.2"), a dist-tag (e.g. "latest", "next"), or omit for latest.' },
      },
      required: ['name'],
    },
  },
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case 'list_versions':
      return listVersions(args.name as string, (args.limit as number) ?? 20);
    case 'get_version_info':
      return getVersionInfo(args.name as string, (args.version as string) ?? 'latest');
    default:
      throw new Error(`Unknown tool: ${name}`);
  }
}

function missingPackage(packageName: string): string {
  return `user_error: npm has no package named "${packageName}". Names are exact and case-sensitive (scoped packages need the @scope/ prefix) — find the right one with npm_search_packages.`;
}
...
async function searchPackages(query: string, limit: number) {
  ...
  const res = await pwFetch(`${REGISTRY}/-/v1/search?${params}`);
  if (!res.ok) throw await httpError(res, 'npm search error');
...
async function getPackage(packageName: string) {
  const res = await pwFetch(`${REGISTRY}/${encodeURIComponent(packageName)}/latest`);
  if (!res.ok) {
    // A 404 on a caller-supplied name is the caller's identifier, not our
    // defect — the user_error: prefix routes it there and the gateway strips
    // the token before the caller sees the message (fleet #409).
    if (res.status === 404) throw new Error(missingPackage(packageName));
    throw await httpError(res, 'npm registry error');
  }
...
async function getDownloads(packageName: string, period: string) {
  ...
  if (!res.ok) {
    if (res.status === 404) throw new Error(missingPackage(packageName));
    throw await httpError(res, 'npm downloads error');
  }
...
async function listVersions(packageName: string, limit: number) {
  const res = await pwFetch(`${REGISTRY}/${encodeURIComponent(packageName)}`);
  if (!res.ok) {
    if (res.status === 404) throw new Error(missingPackage(packageName));
    throw await httpError(res, 'npm registry error');
  }
  ...
  const entries = Object.keys(versions)
    .map((v) => ({
      ...
      deprecated: typeof versions[v].deprecated === 'string' ? versions[v].deprecated : null,
    }))
  ...
  return {
    ...
    last_modified: time.modified ?? null,
    versions: entries.slice(0, size),
  };
}

async function getVersionInfo(packageName: string, version: string) {
  const ver = String(version ?? 'latest').trim() || 'latest';
  const res = await pwFetch(`${REGISTRY}/${encodeURIComponent(packageName)}/${encodeURIComponent(ver)}`);
  if (!res.ok) {
    if (res.status === 404)
      throw new Error(`user_error: npm has no ${packageName}@${ver} — unknown package, version, or dist-tag. See npm_list_versions for the versions that exist.`);
    throw await httpError(res, 'npm registry error');
  }
  ...
  const data = (await res.json()) as {
    ...
    engines?: Record<string, string>;
    deprecated?: string;
    dist?: { tarball?: string; integrity?: string; unpackedSize?: number; fileCount?: number };
  };
  ...
  return {
    ...
    homepage: data.homepage ?? null,
    repository: repoUrl,
    deprecated: typeof data.deprecated === 'string' ? data.deprecated : null,
    engines: data.engines ?? {},
    dependencies: data.dependencies ?? {},
    devDependencies: data.devDependencies ?? {},
    peerDependencies: data.peerDependencies ?? {},
    dist: data.dist
      ? {
          ...
          unpacked_size: data.dist.unpackedSize ?? null,
          file_count: data.dist.fileCount ?? null,
        }
      : null,
  };
}
